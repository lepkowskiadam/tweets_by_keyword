...
def test_pw_hash(app, user, password, expected):
...
def test_set_pw(app, user, password, expected):
...
def test_get_followed_users(app, test_client):
...
def test_clear_follow_list(app, test_client):
    login(test_client, 'test_user', 'test_pw')
    user = User.query.filter_by(username='test_user').first()
...
def test_registration_form(app, test_client, username, email, password, expected_msg, expected_users):
...
def test_login_form(app, test_client, username, password, expected_msg, expected_status):
...
def test_add_followed(mock_handler, app, test_client):
...
@patch.object(TwitterHandler, 'verify_user_exists', return_value=True)
def test_multiple_followers(mock_handler, app, test_client):
...
def test_remove_followed(app, test_client, user):
    login(test_client, 'test_user', 'test_pw')
    followed = Followed.query.filter_by(follower=user).all()
    assert len(followed) == 1
...
@patch.object(TwitterHandler, 'verify_user_exists', return_value=True)
def test_clear_follow_form(mock_handler, app, test_client):
    login(test_client, 'test_user', 'test_pw')
...
def test_tweets_from_followed(mock_tweets, app, keyword, expected):
    user = User.query.filter_by(username='test_user').first()
